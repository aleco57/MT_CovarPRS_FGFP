...
h2files <- list.files(file.path(data.path, "greml/greml_out/h2out_cor")) %>% str_subset(".hsq")

h2_estimates <- c()
for(h2filename in h2files){
  h2file <- read.table(file.path(data.path, "greml/greml_out/h2out_cor", h2filename), header = T, row.names = 1, sep = "\t", fill =T)
...
write.table(h2_estimates, file = file.path(data.path, "greml/greml_out/h2out_cor/clean_out.txt"), row.names = F, quote = F, sep = "\t")
