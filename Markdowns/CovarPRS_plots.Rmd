...
obs_est %>%
  ...
  # Create the heatmap
  ggplot(aes(x = covar_name, y = mt_clean, fill = factor(sig))) +  # Convert sig to a factor for discrete color mapping
...
obsest_prsest %>%
  ...
  # Create the heatmap
  ggplot(aes(x = covar_name, y = mt_clean, fill = sig)) +  # Use the 'sig' column directly for fill
...
obsest_prsest %>%
  ...
  tidyr::complete(covar_name, sig = c("Beta_Agree", "Beta_Disagree"), fill = list(count = 0)) %>%
...
cor_beta %>%
  ...
  select(mt_clean, covar_name, fgfp_estimate, ci95_fgfpcor, fgfp_p.value, rg_GREML, rg_se, mbgbugs, mbg_estimate, ci95_mbgcor, mbg_p.value) %>%
