...
```{r Fig8, fig.width=15, fig.height=20, fig.align='center', out.height="70%", out.width="90%", fig.cap="Heritability estimated by GREML for our microbiome traits with evidence of association with covariate from previous analysis. The heritability has been recalculated before and after adjustment of covariate PRS."}
...
#How many do we need of the unadjusted?
bug_counts <- splitdf$Adjusted$bug %>% table() %>% as.data.frame()

for (i in 1:nrow(bug_counts)){
  if(bug_counts[i,2] > 1){
    bug <- bug_counts[i,1]
    freq <- bug_counts[i,2] - 1
    index <- which(splitdf$Unadjusted$bug == bug)
    
    duplicated_row <- splitdf$Unadjusted[index,]
    duplicated_rows <- duplicated_row[rep(1, freq), , drop = FALSE]
    
    splitdf$Unadjusted <- rbind(splitdf$Unadjusted, duplicated_rows)
  }
}


#Now add in multiple adjustment
splitdf$Adjusted <- mutate(splitdf$Adjusted, covar_name = ifelse(covar2 == "", covar_name, "MultipleCovar"))

#Reorder and add in the same covar1 term
splitdf$Unadjusted <- splitdf$Unadjusted[match(splitdf$Adjusted$bug, splitdf$Unadjusted$bug), ]

splitdf$Unadjusted <- mutate(splitdf$Unadjusted, covar_name = splitdf$Adjusted$covar_name)


#Now bind back together
h2forplot <- rbind(splitdf$Adjusted, splitdf$Unadjusted)

#Change the ordering for the groups
h2forplot$covar_name <- factor(h2forplot$covar_name, 
                                levels = c(setdiff(unique(h2forplot$covar_name), "MultipleCovar"), "MultipleCovar"))
...
ggforestplot::forestplot(
  ...
  labs(colour = "h2 Adjustment Type") 
