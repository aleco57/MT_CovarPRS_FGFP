...
obsest_prsest %>%
  ...
  # Add labels and titles
  labs(title = "Bar chart showing agreement vs disagreement in beta directionality between observational and PRS results",
       x = "Covariate",
       y = "Count of Outcomes") +
...
```{r}
#obs_prs_cor_betacor <- filtered_data %>% select(-std.error_obs, -std.error_prs)
#
#obs_prs_cor_betacor$mt <- gsub("Res", "", obs_prs_cor_betacor$mt)
#
#obs_prs_cor_betacor <- merge(obs_prs_cor_betacor, 
#                             naming_map[,c("term", "covar_name")],
#                             by = "term")
#
#obs_prs_cor_betacor <- obs_prs_cor_betacor %>% select(-term)
#
#obs_prs_cor_betacor <- merge(lm_out,
#      naming_map[,c("prs", "covar_name")], 
#      by.x = "covar", by.y = "prs") %>% 
#  mutate(type = "BetaGWAS_Correlation") %>%
#  rename(mt = bug) %>%
#  select(mt, type, estimate, std.error, covar_name) %>%
#  rbind(obs_prs_cor_betacor)
#
##Remove diversity metrics from the plot
#sig_corbetas_nodiv <- sig_corbetas[!grepl("Div_", sig_corbetas$bug),]
#
#obs_prs_cor_betacor_filt <- obs_prs_cor_betacor %>% filter(mt %in% sig_corbetas_nodiv$bug)
#
#obs_prs_cor_betacor_filt <- obs_prs_cor_betacor_filt[1:3,]
#
##Forest plot for the observational and PRS estimate
#ggforestplot::forestplot(
#  df = obs_prs_cor_betacor_filt,
#  name = mt,
#  estimate = estimate,
#  se = std.error,
#  colour = type,
